...
test('verifies running scores via UI for cresta-yorra-2006.gcg', async ({ page }) => {
  ...
  for (let moveIndex = 0; moveIndex < gcg.moves.length; moveIndex++) {
    ...
    expect(player1Score, `Move ${moveIndex}: ${gcg.player1.name}`).toBe(expectedScores[gcg.player1.nickname])
    expect(player2Score, `Move ${moveIndex}: ${gcg.player2.name}`).toBe(expectedScores[gcg.player2.nickname])
  }
})
